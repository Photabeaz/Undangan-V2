...
try {
    ...
    // 1. PROSES AJAX / JSON REQUEST (Jika dikirim via fetch JS)
    ...
    // 2. PROSES FORM HTML BIASA (Jika JavaScript dimatikan/fallback POST biasa)
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_ucapan'])) {
        ...
        if (!empty($nama) && !empty($pesan)) {
            ...
            header("Location: " . $_SERVER['PHP_SELF'] . (isset($_GET['to']) ? '?to=' . urlencode($_GET['to']) : '') . "#guestbook");
            exit;
        }
    }

    // 3. PROSES AMBIL DATA (Jika diakses via GET request JSON oleh JavaScript `fetchWishes()`)
    if (isset($_GET['action']) && $_GET['action'] === 'get_wishes') {
        ...
        // Format waktu agar mudah dibaca di frontend
        foreach ($ucapan_list as &$u) {
            $u['waktu'] = date('d M Y, H:i', strtotime($u['waktu']));
        }

        header('Content-Type: application/json');
        echo json_encode($ucapan_list);
        exit;
    }

    // 4. PROSES AMBIL DATA UTAMA (Untuk inisialisasi awal halaman PHP)
    $stmt = $pdo->query("SELECT * FROM ucapan ORDER BY id DESC");
    $ucapan_list = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <style>
        ...
        /* Styling transisi mulus pada ombak SVG */
        .wave-divider {
            position: absolute;
            bottom: 0;
            left: 0;
            ...
            pointer-events: none;
            z-index: 15;
            transform: translateY(1px); /* Mencegah celah putih 1px di beberapa browser */
        }
        .wave-divider svg {
            position: relative;
            display: block;
...
<body class="font-sans text-textDark antialiased locked">
    ...
    <div id="main-content" class="hidden relative">
        ...
        <section class="relative w-full h-screen overflow-hidden flex items-center justify-center">
            ...
            <!-- DESAIN BARU: SVG Layer Ombak Bertumpuk -->
            <div class="wave-divider">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
                    <!-- Layer ombak belakang (transparan) -->
                    ...
                    <!-- Ombak pekat/utama (warna solid cream #fdfbf7 menyesuaikan background section berikutnya) -->
        ...
        <!-- Pastikan section ini tidak memiliki padding top berlebih yang mengganggu ombak -->
    ...
    <script>
        ...
        // 5. Render Galeri Berisi Foto Statis Elegan
        const galleryContainer = document.getElementById('gallery-container');
        const sampleImages = [
            'foto/ph (1).jpg', 'foto/ph (2).jpg', 'foto/ph (3).jpg', 'foto/ph (4).jpg',
            'foto/ph (5).jpg', 'foto/ph (6).jpg', 'foto/ph (7).jpg', 'foto/ph (8).jpg',
            'foto/ph (9).jpg', 'foto/ph (10).jpg', 'foto/ph (11).jpg', 'foto/ph (12).jpg',
            'foto/ph (13).jpg', 'foto/ph (14).jpg', 'foto/ph (15).jpg',
        ];
        ...
        // Fungsi Mengambil Data via AJAX (Fetch API)
        async function fetchWishes() {
            try {
                ...
                console.log('Mode Preview atau server belum aktif.');
            }
        }

        // Fungsi Mengirim Ucapan via JSON POST
        async function submitWish(e) {
            ...
            try {
                if (window.location.protocol === 'blob:' || window.location.protocol === 'about:') {
                    alert("Mode Preview: Fitur simpan data memerlukan Local Server (Laragon/XAMPP).");
                    return;
                }
